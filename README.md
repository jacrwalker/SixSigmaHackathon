...
Hospitals are incredibly complex systems with numerous moving pieces. In our hospital system, patients are complaining of wide and irregular gaps between provider and nurse check-ins, leading to worse health outcomes and potentially longer stays in our inpatient care units. Our goal is to develop a tool to reduce wait times between patients by creating a scale that prioritized patients with the highest severity scores without leaving anyone behind. 

This tool is designed for use by hospital units and/or floors to manage patient wait times. Our solution also aims to be as simple as possible to prevent any undue burden on providers and nursing staff. 
...
A severity rating (1–100, with 1 being the patient is in incredible condition, and 100 being the patient is in critical condition)
